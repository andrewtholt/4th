\ 4tH Simple Generator - Copyright 2005,2011 J.L. Bezemer
...
: mysql-field 
  ...
  begin over over chars + c@ [char] ( <> over r@ < and while 1+ repeat
  r> min
;
...
: PrintFormFields
  ...
  1 begin                              \ two colums per row
    margin                             \ print the label
    ." CellLabel ('<b>" fields>s 2dup 2dup type ." </b>');" cr
    margin                             \ create the <FORM> field
...
: BeginProgram                         ( --)
  s| <?php| typeln
  s| // Dit file is gegenereerd door SimpleGen, Copyright 2005,2009 - Hans Bezemer| typeln cr
  s| require ("StdEnv.lib");| typeln
  s| require ("SQL.lib");| typeln
  s| require ("Button.lib");| typeln
  s| require ("Cells.lib");| typeln
  s| require ("Simple.lib");| typeln cr
;
...
: EndProgram                           ( --)
  ...
  margin s| CellHidden ('| type type ." ', $" type ." );" cr 
  margin s| SimpleHidden ();| typeln
